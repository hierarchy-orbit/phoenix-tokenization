...
contract PaymentSystem is BONDS_PARAMS, UNITS_PARAMS, TOKEN_PARAMS {
        ...
        public
    {
        ...
        if (_paymentForInvestor > 0) {
            require(_transferPhoenixToken(msg.sender, _paymentForInvestor), "Error while releasing Tokens for interest payment");
        }
        ...
        if (stoType == STOTypes.BONDS) {
        	...
        	if (_capitalPayment > 0) {
            	require(_transferPhoenixToken(msg.sender, _capitalPayment), "Error while releasing Tokens for capital payment");
        	} 
    ...
    function _transferPhoenixToken(address, uint256) private returns(bool);
