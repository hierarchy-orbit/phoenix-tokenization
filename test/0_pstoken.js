...
contract('Testing PSToken', function (accounts) {
...
/*  it('PhoenixIdentity identities created for owner', async() => {
  ...
  describe('Checking PSToken functionality (SHARES)', async() =>{


    it('PSToken can be created', async () => {

      newToken = await PSToken.new(
          1, // Token Id
          0, // STO Type (0: Shares, 1: Units, 2: Bonds)
          web3.utils.stringToHex("PhoenixSecurityToken"), // Name
          "Phoenix Security", // Description
          web3.utils.fromAscii("HTST"), // Symbol
          18, // Decimals
          instances.PhoenixToken.address, // PhoenixToken address
        ...
        console.log("      PSToken Address", newToken.address)
        console.log("      Token owner", users[9].address)
    })
    
    it('PSToken exists', async () => {
    ...
    it('Appoint token', async () => {
      await instances.TokenRegistry.appointToken(
        ...
        web3.utils.fromAscii('Phoenix Security'), // token name
    ...
    it('PSTBuyerRegistry - add buyer 1', async () => {
    ...
    it('PSTBuyerRegistry - add buyer 2', async () => {
      await instances.BuyerRegistry.addBuyer(
        '3', // EIN
        'Peter', // first name
        'Tester', // last name
    ...
    it('PSTServiceRegistry - add legal advisor', async () => {
    ...
    it('PSTServiceRegistry - get service', async () => {
      ...
      console.log("      PSTServiceRegistry service", web3.utils.toAscii(_serviceCategory))
    })
    ...
    it('PSTBuyerRegistry - add kyc service for buyer 1', async () => {
    ...
    it('PSTBuyerRegistry - get kyc service for buyer 1', async () => {
      _kycProviderForBuyer = await instances.BuyerRegistry.getKycServiceForBuyer(
        '2', // buyer EIN
        newToken.address, // token address
        {from: users[9].address} // token owner
      )
      console.log("      PSTBuyerRegistry kyc service", _kycProviderForBuyer)
    })

    it('PSTBuyerRegistry - add kyc service for buyer 2', async () => {
      await instances.BuyerRegistry.addKycServiceToBuyer(
        '3', // buyer EIN
        newToken.address, // token address
        '9', // service provider EIN
    ...
    it('PSTBuyerRegistry - get kyc service for buyer 2', async () => {
      _kycProviderForBuyer = await instances.BuyerRegistry.getKycServiceForBuyer(
        '3', // buyer EIN
        newToken.address, // token address
        {from: users[9].address} // token owner
    ...
    it('PSToken set MAIN_PARAMS', async () => {
      console.log("      Startconfiguring token")
      await newToken.set_MAIN_PARAMS(
        web3.utils.toWei("0.1"), // phoenixPrice: 1 ether = same price
    ...
    it('PSToken activate Prelaunch', async () => {
      await newToken.stagePrelaunch({ from: users[9].address });
    })

    it('PSToken activate Presale', async () => {
      await newToken.stagePresale({ from: users[9].address });
    })

    it('PSToken activate Sale', async () => {
      await newToken.stageSale({ from: users[9].address });
    })


    it('PSToken adds EIN 2 to whitelist', async() => {
      await newToken.addWhitelist(["2"],
        { from: users[9].address })
    })

    it('PhoenixToken user 2 approves 1M PhoenixTokens for PSToken', async() => {
      await instances.PhoenixToken.approve(
        newToken.address,
        web3.utils.toWei("1000000"),
        { from: users[1].address })
    })

    it('PSTBuyerRegistry - get buyer data - kyc status: false', async () => {
    ...
    it('PSToken Reverts buy for EIN user 3', async () => {
    ...
    it('PSTBuyerRegistry - get buyer data - kyc status: true', async () => {
      _buyerKycStatus = await instances.BuyerRegistry.getBuyerKycStatus(
        '2',
        {from: users[9].address}
      )
    ...
    it('PSToken buyTokens from EIN user 2', async () => {
    ...
    it('PSToken buyTokens again from EIN user 2', async () => {
      await newToken.buyTokens(
          web3.utils.toWei("24"),
          { from: users[1].address })
    })

    it('PSToken activate Lock', async () => {
      await newToken.stageLock({ from: users[9].address });
    })

    it('PSToken activate Market', async () => {
    ...
    it('PSToken reverts transfer 1.2 PSTokens to Account 2', async () => {
      await truffleAssert.reverts(
        newToken.transfer(
          users[2].address,
    ...
    it('PSToken transfer 1.2 PSTokens to Account 2', async () => {
    ...
    it('Setting oracle address', async() => {
      await newToken.addPhoenixOracle(
        users[5].address,
        { from: users[9].address })
    })

    it('Oracle notifies profits of 5 Phoenixs for this period', async() => {
    ...
    it('User claims payment 1', async() => {
      ...
      console.log("PhoenixToken Balance user 1 AFTER:", web3.utils.fromWei(await instances.PhoenixToken.balanceOf(users[1].address)))

    })


    it('Oracle notifies profits of 4 Phoenixs for this period', async() => {
    ...
    it('PSToken transfer 0.8 PSTokens to Account 2, to decrease his participationRate at period 4', async () => {
